...
class SSHConnection(object):
    ...
    def connect(self):
        cmd_line = ('ssh -o "ProxyCommand corkscrew 127.0.0.1 8080 %s %s" -o ConnectTimeout=5 -CND %d %s@%s -p %s' % (self.ip_address, self.port, self.listen_port, self.username, self.ip_address, self.port))
        self.ssh = pexpect.spawn(cmd_line)
        print cmd_line
        # Print ssh output:
        ...
        for i in range(MAX_WAIT_SECONDS):
            try:
                ...
                if i < MAX_WAIT_SECONDS - 1:
                    ...
                    raise
        print '\nSOCKS5 forwarded %s:%d' % (self.listen_address, self.listen_port)
...
#        cmd_line = ('./ssh -C -D %s:%d -N -p %s -z -Z %s %s@%s' % (self.listen_address, self.listen_port, self.port, self.obfuscate_keyword, self.username, self.ip_address))
#        return cmd_line
