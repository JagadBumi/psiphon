...
def connect_to_server(data, relay, bind_all, test=False):
    ...
    if handshake_performed:
        ...
        try:
            ...
            print(str(e).split(' ')[0].replace("'","") + ' Connected ' + str(server.extended_config['region']))
    with open('servers.dat','r') as serv_file:
        tempdata=json.loads(serv_file.read())
    ...
    if test:
        ...
        if server.extended_config['region'] == 'US':
            REG = 'United States'
        elif server.extended_config['region'] == 'GB':
            REG = 'United Kingdom'
        elif server.extended_config['region'] == 'NL':
            REG = 'Netherlands'
        else:
            REG = server.extended_config['region']
        print("Server Country : " + str(REG))
...
def connect(bind_all,copy="", test=False):
    ...
    while True:
        ...
        os.system('clear')
        print('Connect to ' + str(ob["region"]) + ' > ' + str(ob["ipAddress"]) + ':' + str(ob["sshPort"]))
